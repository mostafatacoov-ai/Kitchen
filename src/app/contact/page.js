...
import { Upload, Send, Loader2, CheckCircle, XCircle, X } from 'lucide-react';
    ...
    try {
      ...
      if (result.success) {
        setModal({
          ...
          message: 'شكراً لتواصلك معنا. سيقوم فريقنا بمراجعة طلبك والتواصل معك في أقرب وقت ممكن.',
        });
        e.target.reset();
      } else {
        setModal({
          type: 'error',
          title: 'حدث خطأ',
          message: result.error || 'يرجى التحقق من بياناتك والمحاولة مرة أخرى.',
        });
      }
    } catch (err) {
      console.error(err);
      setModal({
        ...
        message: 'تعذّر الاتصال بالخادم. يرجى التحقق من اتصالك بالإنترنت والمحاولة مرة أخرى.',
      });
    } finally {
              ...
              <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
                {isSubmitting ? (
                  <>
                    ...
                    <span>{t('contact.sending') || 'جاري الإرسال...'}</span>
                  </>
                ) : (
                  <>
